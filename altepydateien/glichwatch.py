...
# grenzlisten
temporotgrenzlist = np.full(100, 35)
luftdruckogrenzlist = np.full(100, 70)
co2orotgrenzlist = np.full(100, 1000)
lautorotgrenzlist = np.full(100, 70)
hellugelbgrenzlist = np.full(100, 100)

tempurotgrenzlist = np.full(100, 20)
luftdruckugrenzlist = np.full(100, 35)
co2ogelbgrenzlist = np.full(100, 400)
lautogelbgrenzlist = np.full(100, 35)
hellurotgrenzlist = np.full(100, 40)
...
class Warnungen(QWidget):
    ...
    def warnungTemphoch(self):
        if tempwarnungen == True:
            self.textLabel.setText(
                "<html><head/><body><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">Es ist zu heiß</span></p><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">im Raum!</span></p></body></html>")
            self.show()

    def warnungenTempniedrig(self):
        if tempwarnungen == True:
            self.textLabel.setText(
                "<html><head/><body><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">Es ist zu kalt</span></p><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">im Raum!</span></p></body></html>")
            self.show()

    def warnungLautstaerke(self):
        if lautwarnungen == True:
            self.textLabel.setText(
                "<html><head/><body><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">Es ist zu laut</span></p><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">im Raum!</span></p></body></html>")
            self.show()

    def warnungCo2(self):
        if co2warnungen == True:
            self.textLabel.setText(
                "<html><head/><body><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">Der CO2 Gehalt ist zu hoch</span></p><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">im Raum!</span></p></body></html>")
            self.show()

    def warnungLicht(self):
        if hellwarnungen == True:
            self.textLabel.setText(
                "<html><head/><body><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">Es ist zu dunkel</span></p><p><span style=\" font-size:22pt; font-weight:600; color:#aa0000;\">im Raum!</span></p></body></html>")
            self.show()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.luftfeuchtampel.setStyleSheet(
            'border: 2px solid black; border-radius:60px; background-color: rgb(0,255,0)')
        ...
        self.einfacheAmpelTemperatur.setStyleSheet(
            'border: 2px solid black; border-radius:40px;background-color: rgb(0,255,0)')
       # self.einfacheAmpelFeuchtigkeit.setStyleSheet(
           # 'border: 2px solid black; border-radius:40px;background-color: rgb(0,255,0)')
        self.einfachAmpelCO2.setStyleSheet('border: 2px solid black; border-radius:40px;background-color: rgb(0,255,0)')
        self.einfacheAmpelLaut.setStyleSheet(
            'border: 2px solid black; border-radius:40px;background-color: rgb(0,255,0)')
        self.einfacheAmpelHelligkeit.setStyleSheet(
            'border: 2px solid black; border-radius:40px;background-color: rgb(0,255,0)')
    ...
    def checkifdevcon(self):
        ...
        if deviceconnected == True:
            ...
            self.einfachesistdevcon.setStyleSheet('color: black;background-color : green')
            self.einfachesistdevcon.setText('Device connected')

        else:
            self.istdevcon.setStyleSheet('background-color : red')
            self.istdevcon.setText('Device not connected')
            self.einfachesistdevcon.setStyleSheet('color: black;background-color : red')
            self.einfachesistdevcon.setText('Device not connected')
    ...
    def updatetempce(self):
        ...
        if daten[1] >= 35:
            self.tempampel.setStyleSheet(
                'border: 1px solid black; border-radius:60px; background-color: rgb(255,255,0)')
            self.einfacheAmpelTemperatur.setStyleSheet(
                'border: 0px solid black; border-radius:20px; background-color: rgb(255,255,0)')
            ...
            self.einfacheAmpelTemperatur.setStyleSheet(
                'border: 0px solid black; border-radius:20px; background-color: rgb(0, 255, 255)')
            ...
            #Ampeln im einfachen Modus
            self.einfacheAmpelTemperatur.setStyleSheet('border: 0px solid black; border-radius: 20px; '
                                                       'background-color: rgba(0,255,0, 80)')
            self.einfacheAmpelTemperatur_2.setStyleSheet('border: 0px solid black; border-radius: 20px;'
                                                         'background-color: rgba(255, 255, 0, 80)')
            self.einfacheAmpelTemperatur_3.setStyleSheet('border: opx solid black; border-radius: 20px;'
    ...
    def updateluftfeucht(self):
        ...
        if daten[2] >= 70:
           ...
           # self.einfacheAmpelFeuchtigkeit.setStyleSheet(
                #'border: 0px solid black; border-radius:40px;background-color: rgb(255,255,0)')
            pen = pg.mkPen(color=(255, 255, 0))
            self.luftfeucht_line = self.luftfeuchtw.plot(pen=pen)
        elif daten[2] <= 35:
            self.luftfeuchtampel.setStyleSheet('border: 1px solid black; border-radius:60px;'
                                               'background-color: rgb(0, 255, 255)')
           # self.einfacheAmpelFeuchtigkeit.setStyleSheet(
               # 'border: 0px solid black; border-radius:40px;background-color: rgb(0,255,255)')
            pen = pg.mkPen(color=(0, 255, 255))
            self.luftfeucht_line = self.luftfeuchtw.plot(pen=pen)
        else:
            self.luftfeuchtampel.setStyleSheet('border: 1px solid black; border-radius:60px; '
                                               'background-color: rgb(0,255,0)')
           # self.einfacheAmpelFeuchtigkeit.setStyleSheet(
               # 'border: 0px solid black; border-radius:40px;background-color: rgb(0,255,0)')
            pen = pg.mkPen(color=(0, 255, 0))
            self.luftfeucht_line = self.luftfeuchtw.plot(pen=pen)
        # muss angepasst werden an die Werte
    ...
    def updateco2(self):
        ...
        if daten[3] >= 1000:
            self.co2ampel.setStyleSheet('border: 1px solid black; border-radius:60px;'
                                        'background-color: rgb(255, 255, 0)')
            self.einfachAmpelCO2.setStyleSheet(
                'border: 0px solid black; border-radius:40px;background-color: rgb(255,255,0)')
            pen = pg.mkPen(color=(255, 255, 0))
            self.co2_line = self.co2w.plot(pen=pen)
...
def datenzeugs():
    ...
    def dateianlegen():
        ...
        dateiname = str('savedluftdata/'+zeit + '.txt')
